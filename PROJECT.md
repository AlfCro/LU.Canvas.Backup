...
| Backup storage | Operator-approved path for the active backup runner; `E:\CanvasBackup` was a previous-server path |
...
1. `scripts/canvas-backup.mjs` is approved as the first validation and emergency backup path; broad execution should use approved root account `1`, the `2025-04-01` cutoff, zero-student exclusion, and an explicit resumable output folder on the active backup server.
...
12. Use an operator-approved output root on the server that actually runs the backup. Backup runs require an explicit `--output` or `CANVAS_OUTPUT_DIR` run folder so retries resume in the same directory. `E:\CanvasBackup` was used on another server and must not be assumed here.
...
- [ ] Confirm the backup output root and available storage for the current execution server; `E:\CanvasBackup` was a previous-server path.
