...
27. `.env.example` should point at `https://lu.beta.instructure.com` by default so copied local config starts against beta; production runs should set `CANVAS_BASE_URL` explicitly after operator review.
...
- [x] Default `.env.example` to the Canvas beta URL for local copied config.
