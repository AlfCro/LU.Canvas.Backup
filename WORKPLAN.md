...
- Current focus: Use `scripts/canvas-backup.mjs` as the approved first validation and emergency backup path. The executable fallback and local config now target `CANVAS_BASE_URL=https://lu.beta.instructure.com`, Canvas account `1`, `CANVAS_BACKUP_MODE=both`, `CANVAS_USER_AGENT=LU.Canvas.Backup/1.0 (Local Canvas backup)`, `CANVAS_CREATED_AFTER=2025-04-01`, `CANVAS_MAX_FILE_SIZE_MB=300`, `CANVAS_EXCLUDE_COURSES_WITHOUT_STUDENTS=true`, conservative concurrency values, and the approved local root `E:\CanvasBackup`. `.env.example` recommends the same beta base URL, root-account policy, course name/code exclusions for `test`, `sandlåda`, `sandlada`, `sandbox`, `mall`, `template`, and `demo`, and subaccount-name exclusion for `sandbox`; the script defaults match those documented exclusion lists, with uniform edge-only matching for all configured terms. Backups are organized by direct Canvas account/subaccount under `subaccounts/<account-id>-<account-name>/courses/...`, and each run fetches subaccount metadata into `subaccounts.json` plus per-subaccount `subaccount.json` files. The script requires explicit opt-in for the limited current-user `/api/v1/courses` discovery fallback, sends a configurable `User-Agent` header on HTTP requests, requests `include[]=total_students` during discovery, skips existing non-empty downloads when Canvas omits file sizes, shortens generated content/file paths near the Windows path limit, uses the documented `include[]` calendar-events parameter, redacts obvious token/session fields in error bodies, and always preserves Canvas-returned staff login/SIS identifiers in staff user/enrollment snapshots. The behavior-preserving module split from `CANVAS_BACKUP_REFACTOR_PLAN.md` is implemented under `scripts/canvas-backup/`. The next operational gate is a root account `1` list-only run with the approved filters, followed by review of selected counts, subaccount distribution, and file-size summaries before broad content backup. Attempted to start that run on 2026-05-08, but the current Codex shell reported `E:\` as not ready before any Canvas API call could run; verify/mount `E:\CanvasBackup` in the execution environment first.
...
- Continued manifest review on 2026-05-08 reconfirmed the final list-only counts, zero subaccount metadata errors, and redacted run options for each familiar-area validation scope. Those are historical validation artifacts; the approved broad-run root is now `E:\CanvasBackup`, which has TB-scale space available.
- Next session should: Verify that `E:\CanvasBackup` is mounted and writable from the execution environment, then run a root account `1` list-only selection with the approved `2025-04-01` cutoff, zero-student exclusion, course/subaccount noise filters, and output under `E:\CanvasBackup`. Use `CANVAS_MAX_FILE_SIZE_MB=300` unless explicitly overridden. Do not download Canvas Studio media. Mention Canvas New Quizzes as a known gap in README-style notes, but do not spend time building a large New Quizzes fetch path now.
...
- [x] Add a configurable non-empty Canvas backup `User-Agent` header via `--user-agent` / `CANVAS_USER_AGENT`.
- [x] Point the executable fallback and `.env.example` at the Canvas beta base URL by default.
...
- [x] After adding root account `1`, `E:\CanvasBackup`, and zero-student filter changes, rerun `node --check scripts\canvas-backup.mjs`, `node scripts\canvas-backup.mjs --help`, `node scripts\canvas-backup.mjs --check-config`, and `node --test scripts\canvas-backup.test.mjs`.
- [x] After adding the configurable User-Agent header, rerun `node --check scripts\canvas-backup.mjs`, `node scripts\canvas-backup.mjs --help`, `node scripts\canvas-backup.mjs --check-config`, and `node --test scripts\canvas-backup.test.mjs`.
...
- [x] Confirm local storage location: `E:\CanvasBackup`.
- [x] User confirmed `E:\CanvasBackup` has TB-scale free space available.
- [ ] Verify `E:\CanvasBackup` is mounted and writable from the execution environment; this Codex shell reported `E:\` as not ready on 2026-05-08.
...
- [x] Switched the approved local output root to `E:\CanvasBackup`, with TB-scale space available, while keeping historical validation notes for reference.
...
- [x] Attempted the root account `1` list-only run, but it did not reach Canvas because `E:\` was not ready in the current shell.
- [x] Added configurable User-Agent support with the generic default `LU.Canvas.Backup/1.0 (Local Canvas backup)`, updated `.env.example`/docs, and verified syntax, help, redacted check-config, and Node tests on 2026-05-13.
- [x] Changed both `DEFAULT_BASE_URL` and `.env.example` to `CANVAS_BASE_URL=https://lu.beta.instructure.com` and confirmed a read-only authenticated beta probe succeeds with the configured User-Agent.
...
- [ ] `E:\CanvasBackup` must be reachable from the machine/session that runs the backup; current Codex shell could not access `E:\` on 2026-05-08.
