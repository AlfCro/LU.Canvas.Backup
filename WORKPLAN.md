...
- [x] Point `.env.example` at the Canvas beta base URL by default for copied local config.
...
- [x] Changed `.env.example` to `CANVAS_BASE_URL=https://lu.beta.instructure.com` and confirmed a read-only authenticated beta probe succeeds with the configured User-Agent.
